...
class MiniMaxH3TrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        if enable_aiter_add_swiglu and getattr(self.pipe, "dit", None) is not None:
            from diffsynth.models.minimax_h3_aiter import enable_minimax_h3_aiter_add_swiglu
            enable_minimax_h3_aiter_add_swiglu(self.pipe.dit)
...
def minimax_h3_parser():
    ...
    parser.add_argument("--enable_aiter_add_swiglu", action="store_true", help="Use optional AITER Add+SwiGLU for supported MiniMax-H3 LoRA fc1 adapters; fall back to PyTorch when unavailable.")
    return parser
...
if __name__ == "__main__":
    ...
    model = MiniMaxH3TrainingModule(
        ...
        task=args.task,
        enable_aiter_add_swiglu=args.enable_aiter_add_swiglu,
